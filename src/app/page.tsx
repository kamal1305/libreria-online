...
export default async function Home() {
  ...
  try {
    books = (await db.book.findMany({ where: { status: "PUBLISHED", stock: { gt: 0 } }, orderBy: { createdAt: "desc" }, include: { category: true } })).map((book) => ({ id: book.id, slug: book.slug, title: book.title, author: book.author, price: Number(book.price), condition: book.condition, imageUrl: book.imageUrl, imageAlt: book.imageAlt, category: book.category?.name ?? "Sin género", stock: book.stock, status: book.status, amazonAffiliateUrl: book.amazonAffiliateUrl }));
    reviews = (await db.review.findMany({ where: { status: "PUBLISHED" }, orderBy: { publishedAt: "desc" }, take: 12 })).map((review) => ({ id: review.id, slug: review.slug, title: review.title, author: review.author, synopsis: review.synopsis, content: review.content, rating: review.rating, coverUrl: review.coverUrl, coverAlt: review.coverAlt, amazonAffiliateUrl: review.amazonAffiliateUrl, instagramUrl: review.instagramUrl, publishedAt: review.publishedAt, status: review.status, book: null }));
  } catch {
    books = demoBooks;
    reviews = demoReviews;
  }
